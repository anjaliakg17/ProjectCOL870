...
print("Get laplacians for IFG (IF group) calculations...")
try:
...
ifgModel = gdEncoder(num_layers = args.num_layers, nfeat=args.hidden, nhid=args.hidden, nclass=num_class, heads=args.heads,
            negative_slope=args.negative_slope, concat=args.concat, dropout=args.dropout)
...
for epoch in range(args.epochs+1):
    ...
    ifgModel.eval()
    ifgOutput = ifgModel(output, sim_edge_index, sim_edge_weight)
    # Get validation losses for gd encoder    
    # Label loss
